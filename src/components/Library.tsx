...
export const SponsoredBadge = () => (
  <span className="px-3 py-1 rounded-full text-xs font-bold bg-gradient-to-r from-pink-500 to-purple-500 text-white shadow-lg shadow-pink-500/20 flex items-center gap-1.5">
  ...
  return (
    <div className={`relative bg-[#0c1222] rounded-2xl p-6 transition-all duration-300 hover:-translate-y-1 border ${
      isSponsored 
        ...
        : 'border-white/10 hover:border-blue-500/50 hover:shadow-[0_0_30px_-5px_rgba(59,130,246,0.2)]'
    }`}>
      {isSponsored && (
...
export const CompanyCard = ({ name, openJobs, logoUrl }: any) => (
  <div className="bg-[#0c1222] rounded-2xl p-6 border border-white/10 hover:border-blue-500/50 hover:shadow-[0_0_30px_-5px_rgba(59,130,246,0.2)] transition-all duration-300 text-center group cursor-pointer">
    ...
    <p className="text-sm text-blue-400 mt-1.5 font-medium">{openJobs} vagas abertas</p>
  </div>
);
    ...
    <div className="space-y-3">
      {options.map((opt, i) => (
        <label key={i} className="flex items-center gap-3 cursor-pointer group">
          <div className="relative flex items-center justify-center w-5 h-5 border-2 border-slate-600 rounded-md group-hover:border-blue-500 transition-colors">
            <input type="checkbox" className="peer sr-only" />
            <div className="absolute inset-0 bg-blue-500 rounded-sm scale-0 peer-checked:scale-100 transition-transform duration-200 flex items-center justify-center">
...
export const LanguageSwitcher = () => (
  <div className="relative group">
    ...
    <div className="absolute right-0 mt-2 w-40 bg-[#0c1222] rounded-xl shadow-2xl border border-white/10 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right z-50 overflow-hidden">
      <div className="py-1">
        <button className="w-full text-left px-4 py-2.5 text-sm text-blue-400 bg-blue-500/10 font-medium">Português (BR)</button>
...
export const AdSlot = ({ type, name }: { type: 'leaderboard' | 'sidebar' | 'native' | 'square', name: string }) => {
  ...
  return (
    <div className={`${dimensions[type]} bg-[#080b1a]/50 border-2 border-dashed border-slate-700/50 rounded-2xl flex flex-col items-center justify-center text-slate-500 p-4 text-center relative overflow-hidden group hover:border-blue-500/30 transition-colors`}>
      {/* AD SLOT: ${name} */}
...
export const ProfileProgress = ({ percentage }: { percentage: number }) => (
  <div className="bg-gradient-to-br from-[#0c1222] to-[#131b2f] rounded-2xl p-6 border border-white/10 shadow-xl relative overflow-hidden group">
    {/* Decorative Stitch-inspired blobs */}
    <div className="absolute -right-12 -top-12 w-32 h-32 bg-pink-500/20 rounded-full blur-2xl group-hover:bg-pink-500/30 transition-colors duration-500" />
    <div className="absolute -left-12 -bottom-12 w-32 h-32 bg-blue-500/20 rounded-full blur-2xl group-hover:bg-blue-500/30 transition-colors duration-500" />
    ...
    <div className="relative z-10">
      ...
      <div className="flex justify-between items-end mb-3">
        <span className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-pink-400">
      ...
      <div className="w-full bg-slate-800/50 rounded-full h-3 mb-5 overflow-hidden border border-white/5">
        <div
          className="bg-gradient-to-r from-blue-500 to-pink-500 h-full rounded-full relative"
